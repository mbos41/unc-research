...
def load_merge(date):
	...
	events.columns = ['event3_count', 'event4_count', 'event5_count', 'event6_count', 'event7_count', 'event3_duration', 'event4_duration', 'event5_duration', 'event6_duration', 'event7_duration']
                  	  	
                  	  	
    merge1 = pd.merge(cust_subcalls, agent_profile, left_on='party_answered', right_on='agent', how='left', suffixes=["_cust", "_profile"])
	          
    merge2 = pd.merge(merge1, agent_records, left_on=['call_id', 'record_id', 'party_answered'], right_on=['call_id', 'record_id', 'agent'], how='left', suffixes=["_cust", "_records"])
